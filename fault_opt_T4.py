...
def build_and_solve_fault(cfg: FaultOptimizeConfig, make_plot: bool = False):
	...
	if not Path(INIT_GUESS_FILE).exists():
		raise FileNotFoundError(f"初猜文件不存在: {INIT_GUESS_FILE}")
	...
	# 四段定义：
	# 1) 一级故障前，固定为初猜；2) 一级故障后到一级结束；3) 二级+整流罩段；4) 二级末段（时长可优化）
	T1 = float(cfg.te)
	...
	t_4_guess = np.linspace(T1 + T2 + T3, T1 + T2 + T3 + cfg.T4_guess, N4 + 1)
	...
	opti = ca.Opti()
	T4 = opti.variable()
	opti.subject_to(T4 >= cfg.T4_min)
	opti.subject_to(T4 <= cfg.T4_max)
	opti.set_initial(T4, cfg.T4_guess)
	dt_4 = T4 / N4
	...
	add_rk4_segment_constraints(opti, X2, U2, N2, ode2, dt, t_grid=t_2)
	...
	if ENABLE_SMOOTHNESS:
		...
		opti.minimize(T4 + cfg.w_ctrl * smoothness)
	else:
		opti.minimize(T4)
	...
	t_4_opt = np.linspace(T1 + T2 + T3, T1 + T2 + T3 + T4_opt, N4 + 1)
	...
	# visual.py 期望 X1/U1 是第一大段，所以把故障前和故障后一并拼接到 X1/U1。
	X1_full = np.hstack([np.array(X1), X2_v[:, 1:]])
	U1_full = np.hstack([np.array(U1), U2_v[:, 1:]])
	...
	print(f"优化后第四段时长 T4={T4_opt:.4f}s")
